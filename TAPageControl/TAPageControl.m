...
- (void)updateDotFrame:(UIView *)dot atIndex:(NSInteger)index Acitvie:(BOOL)active
{
    // Dots are always centered within view
    CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self sizeForNumberOfPages:self.numberOfPages].width) / 2);
    CGFloat y = (CGRectGetHeight(self.frame) - self.dotSize.height) / 2;
    if (active) {
        dot.frame = CGRectMake(x, y, self.dotSize.width * self.currentDotWidthRatio, self.dotSize.height);
    }else{
        dot.frame = CGRectMake(x, y, self.dotSize.width, self.dotSize.height);
    }
}
...
- (void)changeActivity:(BOOL)active atIndex:(NSInteger)index
{
    if (self.dotViewClass) {
        ...
        [self updateDotFrame:dotView atIndex:index Acitvie:active];
    }
...
{
    currentDotimage = [currentDotimage stretchableImageWithLeftCapWidth:currentDotimage.size.width * 0.5 topCapHeight:currentDotimage.size.height * 0.5];
